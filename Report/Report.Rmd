...
For the complementary New York City Census dataset, we focused our analysis on housing related variables such as total population, total number of housing units, number of occupied housing units and number of vacant housing units. We processed this data to match the neighborhood stratification used in the Airbnb dataset, aggregating data where neighborhoods were divided into multiple subparts, such as combining 'Bushwick (East)' and 'Bushwick (West)' into a single 'Bushwick' entry. The two datasets were then merged based on neighborhood names. Neighborhoods from the census data without corresponding matches in the Airbnb dataset were excluded to ensure consistency and relevance in our analysis. 
...
# Sum up statistics if one neighbourhood is divided into multiple subparts. E.g.,  Bushwick (East) and Bushwick (West) would be summed up together as Bushwick.
census <- census %>%
  mutate(
...
table1 <- df %>%
  select(!c(NAME)) %>%
  tbl_summary(
    include = c(host_identity_verified, `minimum nights`, `neighbourhood group`, instant_bookable), 
    label = c(host_identity_verified ~ "Host Verified", 
              `minimum nights` ~ "Minimum Nights Requirement",  
...
table2 <- df %>%
  select(!c(NAME)) %>%
  tbl_summary(
    include = c(`review rate number`, `room type`, cancellation_policy, `Construction year`, price, `service fee`, `number of reviews`, `availability 365`, `reviews per month`), 
    label = c(`Construction year` ~ "Construction Year", 
              price ~ "Listing Price", 
              `service fee` ~ "Service Fee", 
              `number of reviews` ~ "Total Number of Reviews", 
              `availability 365` ~ "Listing Available Days", 
...
The processed key variables are summarized in the tables above. Discrete variables are analyzed for their frequency distribution, with percentages. Continuous variables are summarized using their median values and interquartile ranges in the format: Median (Lower Quartile, Upper Quartile).
...
```{r, message=FALSE, echo=FALSE, fig.width=5, fig.height=3, fig.align='center'}
...
kable_styled <- kable(table, caption = "Top 10 Neighbourhoods by Average Price", digits = 2, format = "latex", booktabs = TRUE) %>%
...
Table 3 above identifies the ten neighborhoods with the highest average listing price. In order to better visualize this information geographically, a map by average listing price is displayed below. 


```{r echo=FALSE, results='asis'}

save_widget <- function(widget, file_name, width = 700, height = 500) {
  # Save the HTML widget to a temporary file
  html_file <- tempfile(fileext = ".html")
  saveWidget(widget, html_file, selfcontained = TRUE)
  
  img_file <- paste0(file_name, ".png")
  
  # Use webshot to take a screenshot of the HTML widget
  webshot(html_file, file = img_file, delay = 2, vwidth = width, vheight = height)
...
map <- leaflet(neighbourhoods_price_df) %>%
  addTiles() %>%
...
map_image <- save_widget(map, "airbnb_price_map")

# Embed the image in the R Markdown document
cat("![](", map_image, ")\n")
```
...
# identified the top 20 words in the Airbnb naming.
...
ggplot(filtered_data, aes(x = word, y = price)) +
  geom_boxplot() +
  coord_flip() + 
...
kable_styled <- kable(model_coef, caption = "Linear Model Summary", digits = 3, format = "latex", booktabs = TRUE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
  column_spec(1, bold = TRUE, border_right = TRUE) %>%
  row_spec(0, bold = TRUE, color = "white", background = "#4d79ff")
...
Given the limited explanatory power of our initial model, as reflected by the very low R-squared values, it becomes evident that there are additional factors affecting Airbnb pricing that are not captured within our current dataset. This realization prompts us to broaden our analytical scope and consider external variables that could impact rental prices. Accordingly, we turn to the New York City Census data, which offers insights into housing availability and demographic characteristics at the neighborhood level. By integrating this census data, we aim to investigate how factors such as population density, housing unit availability, and occupancy rates may influence Airbnb pricing strategies. 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=5, fig.height=3, fig.align='center'}
# Population Density vs. Airbnb Price
ggplot(census_with_price, aes(x = total_population, y = average_price)) +
  geom_point() +
...
The plot above indicates a negative correlation between neighborhood population and average Airbnb prices in New York City: as population increases, average prices tend to decrease. This trend might suggest that in densely populated areas, where the supply of potential Airbnb listings is higher due to a greater number of housing units, there is more competition among hosts, leading to lower prices. Additionally, these areas may not command the same premium as less dense neighborhoods, which might offer a more unique or less readily available experience for travelers. The spread of data points around the trend line, however, indicates that population is not the sole determinant of price, and other factors are also at play.

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=5, fig.height=3, fig.align='center'}
# Housing Units vs. Airbnb Price
ggplot(census_with_price, aes(x = HUnits, y = average_price)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue", se = TRUE) +
  labs(title = "Total Number of Housing Units vs. Airbnb Price",
       x = "Total Number of Housing Units",
       y = "Average Airbnb Price")  +
  theme(
  plot.title = element_text(size = 11, hjust=0.5), 
  axis.title.x = element_text(size = 9), 
...
This plot shows a trend where neighborhoods with a higher number of total housing units tend to have lower average Airbnb prices. This pattern is likely correlated with the population size of these areas, as neighborhoods with more housing units generally have larger populations. The abundance of housing may lead to increased competition among Airbnb hosts, which could drive prices down. The downward slope of the best fit line suggests that as housing availability goes up, market saturation might occur, contributing to a decrease in average price due to higher supply. This aligns with economic principles where increased supply, all else being equal, can lower prices.

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=5, fig.height=3, fig.align='center'}
# Vacancy Rates vs. Airbnb Price
census_with_price <- census_with_price %>%
  mutate(vacancy_rate = vacant_HUnits/HUnits)

ggplot(census_with_price, aes(x = vacancy_rate, y = average_price)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue", se = TRUE) +
...
The plot above displays the relationship between neighborhood housing unit vacancy rates and average Airbnb prices. The trend line suggests a relatively flat relationship, indicating that the average Airbnb price does not significantly vary with changes in the vacancy rate across neighborhoods. A potential explanation for this observation could be that the vacancy rate in a neighborhood reflects a balance between available long-term housing and transient accommodations. High vacancy rates may not necessarily drive down Airbnb prices, as they could indicate a lack of demand for permanent residency rather than an abundance of short-term rental opportunities. Conversely, low vacancy rates could imply a healthy demand for housing but may not affect short-term rental pricing if Airbnb listings cater to tourists rather than residents seeking long-term leases. Therefore, we observe that while one might expect higher vacancy rates to correlate with lower rental prices due to increased supply, the specific market for short-term rentals via platforms like Airbnb may operate under different dynamics than traditional housing markets.
...
However, the model's low explanatory power underscores the complexity of the Airbnb pricing mechanism and suggests the need for more comprehensive analysis. Integrating New York City Census data into our investigation has enriched our comprehension of the Airbnb market. We observed a negative correlation between neighborhood population and average Airbnb prices. This observation indicates that areas with higher populations, which likely have more housing units, see increased competition among Airbnb hosts and potentially driving down prices. Similarly, when considering the total number of housing units, a negative relationship with average Airbnb prices was also observed. However, our findings regarding vacancy rates did not show a significant impact on Airbnb prices, suggesting that the short-term nature of Airbnb's business may not be as affected by long-term housing market dynamics as initially thought.

Future research should consider incorporating a broader set of variables and demographic information. Moreover, we can also employ more sophisticated modeling techniques to better capture the multifaceted nature of Airbnb pricing strategies. This analysis lays the groundwork for further investigation into the pricing landscape of Airbnb listings, offering a foundation for hosts, guests, and policymakers to better understand and navigate the sharing economy.
